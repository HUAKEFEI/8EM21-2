...
class Pid_regulator {
public:
	Pid_regulator(float kp, float ki, float kd, float lim_out_h, float lim_out_l) {
		this->kp = kp;
		this->kd = kd;
		this->ki = ki;
		this->lim_out_h = lim_out_h;
		this->lim_out_l = lim_out_l;
	}

	float kp; float ki; float kd; float kf;
	float lim_out_h; float lim_out_l;
	float d_part = 0; float p_part = 0; float i_part = 0;
	float d_integr = 0;
	float db_l = -0.001; float db_h = 0.001;

	//Рассчитайте значение скорости на выходе ПИД-регулятора 计算 PID 控制器输出端的速度值
	float get_pid_value(float error, float dt) {

		float pid_out = 0;
		float out_buf = 0;
		float sub = 0;

		if ((error > this->db_l) && (error < this->db_h)) {
			error = 0.f;
		}
		p_part = kp * error;
		out_buf = p_part + i_part;
		if (out_buf > lim_out_h) {
			pid_out = lim_out_h;
		}
		else if (out_buf < lim_out_l) {
			pid_out = lim_out_l;
		}
		else {
			pid_out = out_buf;
...
public:
	...
	void go_trajectory() {
		...
		while ((time - start_time) <= duration_ms) {
			...
			rad = 2 * PI * (time - start_time) / duration_ms;
			x_cor_task = 0.3f * std::sin(1 * rad + PI / 2);
